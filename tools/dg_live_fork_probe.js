/* ═══════════════════════════════════════════════════════════════════════════
   ...
   驗的是 Bruce 2026-09-20 交辦的項目 1：
     ...
     ① dg.html 的**每一個**即時量測入口按下去都會跳二選一（不是只有一處）
     ② 選「電腦畫面量測」→ 開 dg-measure.html；選「T-CON 自檢畫面量測」
        → 開 dg-selftest.html
     ③ **兩條路拿到的參數逐字相同**（mode／task／dest／what／v），
        差別只有檔名
     ④ 重用分頁時送出的 `dg-measure-task` 訊息兩條路也逐字相同
     ⑤ 兩條路回傳的結果都**正確回填 DG**（第 2 部分／第 3 部分／光學資料比較／
        第 4 部分四個目的地各驗一次）
     ⑥ dg-selftest.html 真的讀得到那四個參數，而且回傳的訊息形狀與
        dg-measure.html 相同
...
(async function main() {
  ...
  for (const kind of ['pc', 'tcon']) {
    ...
    for (const id of ENTRIES) {
      ...
      await new Promise(r => setTimeout(r, 5));
      CHECK(opened.length === 1, '選完才開分頁，而且只開一個（' + kind + ' / ' + id + '）', opened.length);
  ...
  for (const kind of ['pc', 'tcon']) {
    ({ dom, win, opened } = await loadDg());
    // 第一次：開分頁
    win.document.getElementById('dg-btn-live-gray').click();
    await new Promise(r => setTimeout(r, 5));
    win.document.getElementById(kind === 'pc' ? 'dg-btn-pick-pc' : 'dg-btn-pick-tcon').click();
    await new Promise(r => setTimeout(r, 5));
    const w = opened[0];
    ...
    // 第二次：同一條路 → 應該重用（不開新分頁）
    win.document.getElementById('dg-btn-live-prim').click();
    await new Promise(r => setTimeout(r, 5));
    win.document.getElementById(kind === 'pc' ? 'dg-btn-pick-pc' : 'dg-btn-pick-tcon').click();
    await new Promise(r => setTimeout(r, 5));
  ...
  // gray 模式：L 值原樣帶（不是「第幾步」）
  P.dgApplyTask({ task: 9, mode: 'gray', dest: '第 2 部分', what: '白灰階亮度' });
  P.__setRowsForTest([
    { key: 'L0', x: 0.25, y: 0.25, lv: 0 },
    { key: 'L128', x: 0.3127, y: 0.3290, lv: 60 },
    { key: 'L255', x: 0.3127, y: 0.3290, lv: 300 },
    { key: 'L64', x: null, y: null, lv: null },          // 失敗那一階
    { key: 'R', x: 0.64, y: 0.33, lv: 60 },
    { key: 'G', x: 0.30, y: 0.60, lv: 200 },
    { key: 'B', x: 0.15, y: 0.06, lv: 20 }
  ]);
  const g = P.dgRows();
