...
(async function main() {
  ...
  H('7. 卡片改名「畫面測試」＋ 一行定位說明');
  {
    ...
    /* ═══ 🔴 dgself v1.12.0 改判：「不會影響實際量測」那一句已刪除 ═══════════════
       Bruce 2026-09-21：「全部字體大一點，說明文字少一點…要精簡、無腦。」
       ⇒ 被砍掉的是「這裡的調整不會影響下面實際量測的 L0…L255」—— 它講的是
         **不會發生的事**，不是他要做的事。這一輪整頁的刪減都照同一條判準。
       🔴 不是把斷言刪掉了事：這一條換成**正面驗「它真的變短了」**，而且把上限
          從 80 收到 40 —— 否則下一個人把長句子加回來也不會有人發現。 */
    CHECK(!/不會影響/.test(note), '🔴 v1.12.0：「不會影響實際量測」那一句已刪除');
    CHECK(/擺好位置|對位/.test(note), '講了對位是給量測儀擺位用');
    CHECK(note.length <= 40, '🔴 v1.12.0：一句話就好（≤40 字，比 v1.3.0 的 80 更嚴）', note.length);
  }
