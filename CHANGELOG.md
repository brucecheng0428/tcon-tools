...
- **波形區本身沒有變**：清除後可見通道仍是 0 條，canvas 依既有規則（`visibleChs.length === 0`）整塊換成空白提示，所以那 6 列內部訊號**當下一樣看不到**。差別要等使用者把任一條通道叫回來才顯現 —— 那時 v4.35.1 只回來 1 條通道列，本版會同時出現 1 條通道列 ＋ 6 條內部列（實測 canvas 高度 448 vs 570，差 122 px）。
...
1. **C2-N1**：清除後可見通道 **0** 條、canvas `display:none`、空白提示顯示，而 `.wfg-ch-vis` 仍有 **33** 列 —— 通道是被藏起來不是被刪掉。**這一項在兩版都 PASS**，正是 Bruce 指定要證明的「⑦-a 沒有被一起改掉」。
