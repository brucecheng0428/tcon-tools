...
## TCON 波形模擬與取樣 (wfg) v4.37.2 — 2026-08-28 ｜ PATCH ｜ ⚠ 輸出變更

**修「Intel HEX 第 1 行格式不正確」——檔頭的 UTF-8 BOM，以及只算 data 的 checksum**

**判定依據：** 依 `docs/VERSIONING.md` §1 判定表與 R1～R4 逐項判、取最高者 → **PATCH**。

- **R1（修 bug）**：命中。讓「本來就該讀得進來的檔案」讀得進來，不是主動改設計。
- **`⚠ 輸出變更` 要標**：同一份檔案舊版被拒、本版會匯入成功。實測 191 份 hex 語料中 **11 份由「解不出來」變成「解得出來」**。
- **R2 / R3 / R4**：都不適用（不開新波、沒有新增能力、沒有動預設值）。
- **不是 MINOR**：使用者能做的事沒有多一件，是既有能力對這 11 份檔補齊。
- 改了 `wfg.html`、`common/i18n.js`（新增一個 key）、`common/version.js`、`tools/`；`?v=` 一併 bump 成 `20260828wfg4372`。

---

### 🔴 一、先更正 v4.37.1 的診斷

**v4.37.1 修的東西本身是對的，但它不是 Bruce 回報的那個症狀的原因。** 那一版的條目寫著「先講最重要的一句：沒有任何一版把它改壞」與「根因：值域判準」，**在「值域」這件事上仍然成立**（177 份語料誤殺 32 份、逐版重跑全部拒收，都是實測），但它**回答錯了題**：

- v4.37.1 用的是 Bruce **4/2 寄給我的那一份**（`RM81002_…_20260402.hex`，**沒有 BOM**）。那一份確實是被值域判準擋下的，修完就進得去。
- **Bruce 螢幕上真正失敗的是另一份檔**（`RM8100x_JDZ_CSOT15.3…AUPI02/03_20260827.hex`），錯誤原文是 **「Intel HEX 第 1 行格式不正確」** —— 它連 `wfgNbSane()` 都沒走到。

⇒ **這是兩個獨立的 bug，各修各的。** v4.37.1 的條目**原文一字不改**（改寫紀錄不是更新），此處補上更正，避免下一個人以為 v4.37.1 修的就是這個症狀。

**「按過清除就不行 / 先匯入 EM02 就可以」這條線索，實測是誤判 —— 沒有據此改任何一行。**
四情境矩陣（每格都走真的按鈕：Code→匯入→選型號→選檔）：

| 情境 | v4.37.0 | v4.37.1 |
|---|---|---|
| A 直接匯入該 `.hex` | ❌ 失敗 | ✅ 成功 |
| B **清除** → 匯入 | ❌ 失敗 | ✅ 成功 |
| C 先匯入 EM02 `.bin` → 匯入 | ❌ 失敗 | ✅ 成功 |
| D 清除 → EM02 → 匯入 | ❌ 失敗 | ✅ 成功 |

同一版本內四格結果一致 ⇒ **「清除」與「先匯入 EM02」都不是變因**。清除後的狀態也逐項比對過（機種 e503、Line Buffer/FLR 歸零、通道全隱藏），對 code 匯入沒有任何影響。真正的變因是**手上有兩份不同的檔**：一份有 BOM、一份沒有。
...
真檔開頭是 `EF BB BF 3A 30 38 …`；`file(1)` 判為 `UTF-8 (with BOM) text, with CRLF line terminators`。

`wfgCodeDecodeIntelHex()` 逐 byte `String.fromCharCode` 拼字串，BOM 於是變成 **latin-1 的三個字元 `ï»¿`**，而 `trim()` 不會去掉它們（不是 JS 認定的空白）⇒ 第 1 行 `charAt(0)` 是 `ï` 不是 `:` ⇒ 「第 1 行格式不正確」。

**修法：在 bytes 層剝掉開頭的 `EF BB BF`。** 選 bytes 層而不是字串層，是因為 BOM 本來就是位元組層的事實，判斷條件就是它的定義；字串層要比對的 `ï»¿` 是「逐 byte 轉換的產物」，既不可讀也容易寫錯。**只剝檔案開頭這一次** —— 中間任何一行冒出這三個 byte 都是真的壞檔，照舊擋下（有負控制）。行號不受影響（BOM 裡沒有換行）。

**UTF-16 BOM（`FF FE` / `FE FF`）刻意不支援。** UTF-16 每個 ASCII 字元佔 2 bytes，剝掉 2 byte 的 BOM 沒有用 —— 後面整份是夾著 NUL 的字串，要真的做 UTF-16 解碼。剝了只會讓它「看起來被支援了」再用更難懂的方式壞掉。189 份語料沒有任何一份是 UTF-16，**沒有素材可以驗，所以不猜**；但給了一則講清楚是「編碼」問題的訊息（新增 `wfg.codeErrHexUtf16`，三語），而不是沿用「第 1 行格式不正確」。

#### 第二層：剝掉 BOM 之後仍然失敗 —— 只算 data 的 checksum

**這一層是我剝完 BOM 重跑才發現的**，錯誤訊息變成 **「Intel HEX 第 1 行 checksum 不符」**。

標準 Intel HEX 的 checksum 涵蓋整列（len ＋ addr ＋ type ＋ data ＋ cks，總和低位元組為 0）。這份檔的 checksum **只對 data bytes 取二補數，不含 length／address／record type**。逐位元組驗算（第 1 行 `:0800000000FFFFFFFFFFFF0006`）：整列和 `&0xFF = 8`（不為 0）；data 和 `0xFF×6 = 1530`，`1530 & 0xFF = 250`，二補數 `= 6` ＝ 檔案裡的 `06` ✅。

**語料實測（191 份＝189 份信件封存 ＋ Bruce 回報的 2 份）：**

| 類型 | 份數 |
|---|---|
| 每一列都符合標準規則 | **180** |
| 資料列（type 0）全部符合 data-only、非資料列符合標準 | **11** |
| 兩種規則都湊不出一致的解讀 | **0** |

而且這 11 份與**帶 UTF-8 BOM 的 11 份完全是同一批**（語料裡 9 份、Bruce 的 2 份），大小也一致（那 9 份都是 29712 B ⇒ 解出 8192 B；Bruce 的 2 份 14864 B ⇒ 解出 4096 B）。⇒ **BOM 與 data-only checksum 是同一個產生器的兩個特徵，不是檔案壞掉。**

#### 🔴 修法：方言是**整檔**決定的，不是逐列

```
第一趟  只驗結構（冒號開頭／hex 合法／列長 = len + 5），把 record 收下來
        —— checksum 刻意不在這裡驗
第二趟  用標準規則掃全檔 → 全過 ⇒ dialect = 'standard'
        沒全過 → 用 data-only 規則重掃全檔 → 全過 ⇒ dialect = 'data-only'
        兩種都湊不出一致解讀 ⇒ 拒絕（回報標準規則第一個失敗的列號）
第三趟  才組成映像
```

🔴 **絕對不做「逐列各自擇一」**：那等於每一列都有兩次機會，校驗強度直接腰斬。整檔一致則不然 —— 一份標準檔只要有一列 data 被改壞，標準掃描失敗 ⇒ 改用 data-only 重掃 ⇒ 其餘每一列幾乎都不符合 data-only ⇒ 整檔拒絕。

**兩種方言的可分辨性（實測）**：180 份標準檔共 **61697 筆資料列**，其中「碰巧也符合 data-only」的只有 **241 筆（0.39%）**。兩者區分得很開，不是隨便都能互相冒充。

🔴 **一個量出來、不是設計出來的細節：data-only 方言下，非資料列仍用標準規則。** EOF／擴充位址那幾列沒有 data（len = 0），data-only 規則套上去會退化成「cks 必須是 0」，而標準 EOF 是 `:00000001FF`（cks = 0xFF）。實測 Bruce 那份：**513 列 ＝ 512 筆資料列（512/512 符合 data-only）＋ 1 筆 EOF（只符合標準）**。所以「整份都用 data-only」是解不出來的。

🔴 **沒有用 BOM 或檔名去猜「哪一種檔該用哪一種規則」** —— 那是把兩個碰巧同時出現的特徵綁成因果。整檔一致性偵測不需要這個假設。

#### 這份檔實際解出來是什麼（Dispatch 要求實跑，不要假設）

| | |
|---|---|
| `srcFormat` | `hex` |
| `cksDialect` | **`data-only`** |
| `content_len` | **4096**（＝ E503 的唯一合法大小） |
| EDID magic @ 0x0 | **在**（`00 FF FF FF FF FF FF 00`） |
| EDID 128-byte checksum | **通過** |
...
| 項目 | 結果 |
|---|---|
| **Bruce 螢幕上失敗的那兩份**（AUPI02 / AUPI03，BOM ＋ data-only） | ✅ **匯入成功**（真瀏覽器、真按鈕），CKS 徽章 **`0x04E3E9`** —— **與檔名裡的 `CKS(5289_4E3E9h)` 一致**；帶進來的值：1920×1200、HT 2180、VT 1235、60 Hz、**Dual Gate**、First Line Read 12、10 條數位訊號 enable |
| **對照組：4/2 那份**（無 BOM、純標準） | ✅ 仍然成功，CKS `0x046CC3`（與檔名 `46CC3h` 一致）、1920×1080、Single Gate、FLR 4、7 條訊號，**沒有壞掉** |
| 三語未翻譯 key（新增了一個 i18n key） | `zh-TW` / `zh-CN` / `en` 皆 `pass: true`、`hits: []` |
| **191 份語料解碼率** | v4.37.0：180 成功 / 11 失敗（全是「第 1 行格式不正確」）→ **v4.37.2：191 / 0** |
| 真檔語料判定（`WFG_NB_CODE_DIR`） | 掃到 191 份，**解碼失敗 0**，E503 形狀 **179** 份（v4.37.1 只認得 177 —— 少的那 2 份就是 Bruce 的），**誤殺 0** |
| **🔴 負控制 · 整檔一致性**（分水嶺） | **一列 data-only ＋ 一列標準 ⇒ 拒絕**。逐列擇一會放行這份、整檔一致會擋下 —— 這一項就是兩種設計的差別 |
| **負控制**（放寬不可以拆掉偵測能力） | data 被改一 byte（標準檔／data-only 檔皆然）❌拒；checksum 欄位被改 ❌拒；第 1 行不是冒號 ❌拒；列長與 `len` 不符 ❌拒；缺 EOF ❌拒；**BOM 之外的位置冒出 `EF BB BF`** ❌拒；UTF-16 BOM ❌拒（且訊息講明是編碼問題）；`.bin` 不走 hex 解碼 ✅原樣 |
| 方言判定 | 標準檔回報 `cksDialect: 'standard'`、變體檔回報 `'data-only'`（不是「碰巧過關」） |
| 五支機械檢查 | 全部 rc=0 |

### 四、常設檢查補上「解碼層」（同一個破口的第三次）

`tools/check_nb_code_import.js` 新增 ③④ 兩節（12 項），並修掉它自己的一個盲點：

🔴 **原本「選配真檔語料」那一段自己寫了一份簡易 Intel HEX 解碼器** —— 於是它**看不到 BOM 這個 bug**：自己的解碼器不剝 BOM，那 11 份真檔解出來長度不對，被當成「不是 E503 形狀」**靜默跳過**。抄一份實作就會有兩份不同步的版本，而且不同步的方向剛好是「檢查看不到產品的缺陷」。已改為呼叫產品的 `wfgCodeToImage()`。

**破口的演進，寫下來給下一個人：**

| 次 | 驗收只做了哪一面 | 漏掉什麼 |
|---|---|---|
| ① | 只驗「壞檔會被拒絕」 | 沒驗「真檔會被接受」→ 值域判準誤殺 18% |
| ② | 只驗純函式吃 bytes | 沒驗帶 UI 狀態的完整路徑（本輪實測證明 UI 狀態無影響，但當時無從得知） |
| ③ | 只驗判定層（`wfgNbSane`） | 沒驗解碼層（`wfgCodeDecodeIntelHex`）→ BOM 與 checksum 變體 |

**這支檢查自己的鑑別力**：同一支腳本 ＋ 同一批真檔跑在 **v4.37.1 未修的 `wfg.html`** 上 → 14 項不通過（3 項合成案例 ＋ 11 份真檔解碼失敗）、rc=1；跑在本版上 → 全數通過、rc=0。

---

## TCON 波形模擬與取樣 (wfg) v4.37.1 — 2026-08-28 ｜ PATCH ｜ ⚠ 輸出變更

> 🔴 **v4.37.2 事後更正（原文以下一字未改）**：本版修的「值域判準誤殺真檔」是真的（177 份語料誤殺 32 份、逐版重跑全部拒收，皆為實測），但它**不是 Bruce 當時螢幕上那個症狀的原因**。他實際失敗的是另一份帶 UTF-8 BOM 的檔，死在 Intel HEX 解碼器、連 `wfgNbSane()` 都沒走到。兩個獨立的 bug，詳見 v4.37.2。

**修「E503 匯入真實 code 被判成『不是 E503 的 code』」——「值太大就不是這顆」這個前提是錯的**

**Bruce 2026-08-28 回報：**

> 有 bug 了……這個是 E503 的 code。你看一下為什麼 E503 沒辦法匯入這版 code？之前可以，為什麼現在又不行？也把是哪一個步驟改壞的一併找出來。

**判定依據：** 依 `docs/VERSIONING.md` §1 判定表與 R1～R4 逐項判、取最高者 → **PATCH**。

- **R1（修 bug，即使輸出會變仍算 PATCH，但要標 `⚠ 輸出變更`）**：本項命中。這是把行為修回「本來就該是這樣」（一份合法的 E503 code 應該匯得進來），不是主動改設計。
- **`⚠ 輸出變更` 要標**：**同一份檔案在舊版被拒、在本版會匯入成功**，畫面上的 18 條數位訊號、解析度、Frame Rate、Gate Type、First Line Read 全部會被換掉。依 §R1「同一操作序列得到不同結果」⇒ 必須標。實測影響範圍：**177 份真實 E503 檔中有 32 份（18.1%）由「拒絕」變成「接受」**。
- **R2 / R3 / R4**：都不適用 —— 沒有開新波、沒有新增能力（這是既有能力補回來，不是新能力）、沒有動任何預設值。
- **不是 MINOR**：使用者能做的事沒有多一件。「匯入 E503 的 .hex／.bin」這個功能從 v4.14.0 就宣稱存在，本版只是讓它對其中 18% 的檔案真的成立。
- 改了 `wfg.html`、`common/version.js`、`tools/`；`?v=` 一併 bump 成 `20260828wfg4371`。**`common/i18n.js` 沒有改**（沒有新增或修改任何文案）。

---

### 一、🔴 先講最重要的一句：**沒有任何一版把它改壞**

Bruce 的指示是「把是哪一個步驟改壞的一併找出來」。實測結果是**這個前提不成立**，照實回報：

**證據一 —— `git log -S` 逐字比對**：`wfgNbSane()` 這支函式與那一行 `if (s.r_dly > ht || s.f_dly > ht) return false;`，自 **v4.14.0（`16c426f`，2026-08-23，NB 匯入誕生的那一版）**寫下之後，**到 v4.37.0 為止一個字都沒有被改過**（`git log -S` 只命中 `16c426f` 一筆）。

**證據二 —— 逐版實跑同一份檔**（`git archive` 各版本原始碼，直接呼叫產品的 `wfgNbParse()`，不受 UI 改版影響）：

| 版本 | 結果 | 版本 | 結果 |
|---|---|---|---|
| v4.14.0 | ❌ 拒絕 | v4.25.0 | ❌ 拒絕 |
| v4.15.0 | ❌ 拒絕 | v4.34.0 | ❌ 拒絕 |
| v4.16.0 | ❌ 拒絕 | v4.35.0 | ❌ 拒絕 |
| v4.16.2 | ❌ 拒絕 | v4.35.3 | ❌ 拒絕 |
| v4.17.0 | ❌ 拒絕 | v4.36.0 | ❌ 拒絕 |
| v4.22.0 | ❌ 拒絕 | v4.37.0 | ❌ 拒絕 |
| v4.22.2 | ❌ 拒絕 | | |
| v4.23.0 | ❌ 拒絕 | | |

⇒ **這份檔從來沒有匯入成功過。** 「之前可以」很可能指的是**別的檔案** —— 同一批真檔裡有 145/177 份一直都匯得進來，所以「E503 匯入可以用」這個印象是對的，只是那 18% 從一開始就在被靜默擋掉。

**（順帶排除的假設）** Dispatch 端特別點名懷疑「v4.36.0／v4.37.0 的匯入選擇視窗把 code 匯入也一起吃掉了」。**實測不是**：code 匯入走的是 `wfgCodeImport()` → `wfgImpShow()`（型號確認框）→ `wfgCodePickFile()`，與設定檔的 `wfgImpSelShow()` 完全沒有交集；v4.14.0（那兩個視窗都還不存在）拒收同一份檔，直接推翻這個假設。

---

### 二、根因：判定「是不是這顆型號」時用了一條**事實錯誤**的前提

失敗發生在 `wfgNbSane()`，它是 NB 解析的最後一道判定。四條值域判準與它們的原註解：

```js
if (s.r_dly > ht || s.f_dly > ht) return false;              // 「延遲量是一條 line 內的 DCLK 數」
if (s.st_line !== 16383 && s.st_line > vt * 3) return false; // 「×3 已涵蓋 tri-gate」
if (s.sp_line !== 16383 && s.sp_line > vt * 3) return false;
```

**這兩個前提對真實的 E503 code 不成立。** Bruce 那份檔解出來：

| | 值 | 對照 |
|---|---|---|
| EDID（檔案 0x000） | 1920×1080、hblank 270、vblank 60、149.80 MHz → 60 Hz | 完全合法 |
| 計算出的 checksum | `0x046CC3` | **與檔名裡的 `CKS(5CC7_46CC3h)` 一致** ⇒ 解析是對的 |
| 第 4 條訊號 **VST2** | `F_DLY = 0xFFFF`、`SP_LINE = 16000` | 被判「超過 HTOTAL 2190／VTOTAL×3」而整份拒收 |

**語料實測（177 份 4096 B、EDID 合格的真實 E503 EEPROM 檔，取自信件封存）：**

- **32 份（18.1%）被拒**，而且**每一份的原因都只是這四條值域判準，沒有第二種原因**。
- 被判超界的 `F_DLY` 值是**連續分佈**，不是解析錯位會有的指紋：
  `2487 / 4168 / 5500 / 6228 / 6801 / 9366 / 10392 / 10905 / 20264 / 20520 / 25409 / 27461 / 28601 / 29884 / 30910 / 31166 / 40815 / 52711 / 53352 / 65535（18 份）`
- 啟用訊號的實際最大值：**R_DLY 50923、F_DLY 65535、ST_LINE 14820、SP_LINE 16382**。

⇒ 這些 register 本來就會用到整個值域。**「延遲不會超過一條 line」是我們自己編的，不是晶片的規則。**

---

### 三、修法：把「值的大小」整組從型號判定裡移除

```diff
-  var ht = fr.hactive + fr.hblank, vt = fr.vactive + fr.vblank;
   var ne = 0;
...
-    ne++;
-    if (s.r_dly > ht || s.f_dly > ht) return false;
...
保留的三條：**解析度合理**、**XSTB 必須 enable**、**至少 4 條訊號 enable**。

**為什麼不是「改成比較寬鬆的上限」**：任何新的倍數都是猜的，下一份合法的檔就會被它擋掉 —— 同一個錯再犯一次。而若改成「用 register 位元寬當上限」則是**恆真式**：`wfgNbDecodeChunk()` 讀出來的 R_DLY/F_DLY 已經是 16 bit、ST/SP 已經遮成 14 bit（E501/E503）或 16 bit（EN01），與 `WFG_GPO_BITS_NB` 完全一致。**一個永遠成立的檢查等於沒有檢查**，寫了只會讓人以為還有一道防線。

**拿掉之後靠什麼認型號**（這三條本來就是主力，值域那幾條是附加的）：

1. `m.sizes` 嚴格比對內容長度（E503 = 4096、E501 = 131072）。
2. `wfgNbEdidFrame()`：該型號自己的 `edidOff` 上要有 EDID 的 8 byte magic **而且 128 byte checksum 為 0** —— 隨機命中是 2^-72 等級。
3. 本函式保留的三條。

**值太大不會被靜默吃進去**：匯入後仍會過 `wfgClampGpiosToModel()`，超出該機種位元寬的值會被夾並**逐項列在匯入卡片上**（v4.15.0 起的既有行為）。R_DLY/F_DLY 在 NB 是 16 bit ⇒ 65535 合法、不會被夾，那就是晶片裡真正的數字。

#### 一併更正了一段會誤導人的舊註解

`wfgNbSane()` 上方原本寫著「全部是『物理上不可能』的條件。實測對位址錯位有鑑別力……」。**原文逐字保留**（改寫別人的原話不是更新，是竄改），下面加上兩條更正：

- 「物理上不可能」是錯的判斷（見上）。
- 位址錯位的鑑別力**變弱但沒有消失**（本版實測，同一份 E503 檔）：`delta=0` 仍拒絕 ✅、`base+8` 仍拒絕 ✅、**`delta=0x80` 現在會通過**（移除前會被擋）。**這個代價是知情下接受的**：`delta` 是我們自己表裡的常數、不是使用者輸入，寫錯它是編碼錯誤，而編碼錯誤已由新的機械檢查與「177 份真檔全數解得出正確 CKS」擋住。

---

### 四、驗收破口與補上的機械檢查

🔴 **這個 bug 藏了 14 個版本，根因不只是那條判準寫錯，而是驗收只驗了一面。** 歷次 NB 匯入的驗收都在驗「**壞檔會不會被拒絕**」（交叉餵食、垃圾檔、EDID 壞掉），**從來沒有驗過「真檔會不會被接受」**。單面的驗收對「判準太嚴」這一類錯完全沒有鑑別力。

新增 **`tools/check_nb_code_import.js`**，並**已加進 `tools/hooks/pre-commit`**：

- **① 必須接受**：合成一份結構合法的 E503 EEPROM 映像，帶上真檔實際出現過、且舊版會誤殺的數值（`F_DLY=0xFFFF`、`SP_LINE=16000`、`F_DLY=5500`、`R_DLY=50923`、`ST_LINE=14820`）—— 任何人日後想把值域判準加回去，都會在這裡被擋下。
- **② 必須拒絕**：EDID magic 壞一 byte／checksum 不對／XSTB 沒 enable／只有 3 條 enable／解析度荒謬／全 0x00／全 0xFF，一律要被拒 —— 保證①沒有把型號判別一起拆掉。
- **③ 選配**：`WFG_NB_CODE_DIR=<dir>` 可以拿本機真檔整批跑。**真檔不進版控**（這個 repo 等於 GitHub Pages 公開可抓，客戶 code 帶客戶名、面板型號與真實 EDID）。
- 實作方式是**從 `wfg.html` 直接切出產品函式在 node 裡跑**（三支都是純函式、不碰 DOM），驗的是產品本身而不是另一份抄本，也因此零外部相依。

**這支檢查自己的鑑別力也驗過**：把同一支腳本跑在 **v4.37.0 未修的 `wfg.html`** 上 → ①的 5 條全部 ❌、rc=1（會擋下 commit）；跑在修好的版本上 → 全數 ✅、rc=0。
...
| 項目 | 結果 |
|---|---|
| **Bruce 那份 `.hex`（走真的按鈕：Code → 匯入 → 選 E503 → 選檔）** | ✅ **匯入成功**，無 alert；CKS 徽章 `0x046CC3`（**與檔名裡的 `46CC3h` 一致**）；跳出「匯入完成 — 有 1 項需要你確認」；副標「已匯入 14 條數位信號」 |
| 匯入後的值 | 1920×1080、HTOTAL 2190、VTOTAL 1140、60 Hz、Single Gate、First Line Read 4、PANEL_MODE 1（Normal，與 RD_MODE=0／single 的官方對照表相符）、TCON UI DCLK 89 MHz、7 條數位訊號 enable |
| **真檔語料前後對照**（189 份 `.hex`，其中 177 份是 E503 形狀） | v4.37.0：OK 145 / FAIL 32 → 修好後：**OK 177 / FAIL 0**；**由接受變成拒絕的：0 份（無回歸）** |
| **正控制**（證明測法有鑑別力） | 修法之前就有 145 份可以匯入 ⇒ 不是「全錯」，腳本沒壞 |
| **負控制 · 交叉餵食型號** | E503 4KB → E501A ❌ / E501B ❌ / EN01 ❌；E501 128KB → E503 ❌ / EN01 ❌、→ E501A ✅ / E501B ✅（這兩顆本來就只差 DCLK，設計如此） |
| **負控制 · 垃圾檔** | 全 0x00 ❌ / 全 0xFF ❌ / 偽隨機 ❌（全部在 EDID 那一關就被擋）；EDID magic 壞一 byte ❌；全部訊號 disable ❌ |
| 四支既有機械檢查 ＋ 新增的第五支 | 全部 rc=0 |
