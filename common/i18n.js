...
var I18N = {
  ...
  'pat.outDepthHint':   { 'zh-TW': '10 bit 是給只吃 10-bit 圖、不吃 8-bit 的機台用的。畫面本身仍以 8 bit 產生，存檔時每個通道用 MSB replication 放大成 10 bit（0→0、255→1023，最暗與最亮精準對應）。檔案為 32bpp BI_BITFIELDS（A2R10G10B10）。', 'en': 'Use 10 bit for equipment that only accepts 10-bit images. The pattern itself is still generated at 8 bit; each channel is expanded to 10 bit by MSB replication when saving (0→0, 255→1023, so black and white map exactly). The file is 32bpp BI_BITFIELDS (A2R10G10B10).', 'zh-CN': '10 bit 是给只吃 10-bit 图、不吃 8-bit 的机台用的。画面本身仍以 8 bit 产生，存档时每个通道用 MSB replication 放大成 10 bit（0→0、255→1023，最暗与最亮精准对应）。文件为 32bpp BI_BITFIELDS（A2R10G10B10）。' },
  'pat.outDepthPngNA':  { 'zh-TW': 'PNG 目前只支援 8 bit（PNG 規格沒有原生 10 bit，要靠 16 bit + sBIT，尚未實作）；需要 10 bit 請選 BMP。', 'en': 'PNG currently supports 8 bit only (PNG has no native 10-bit mode; it would need 16 bit + sBIT, not implemented yet). Choose BMP for 10 bit.', 'zh-CN': 'PNG 目前只支持 8 bit（PNG 规格没有原生 10 bit，要靠 16 bit + sBIT，尚未实作）；需要 10 bit 请选 BMP。' },
