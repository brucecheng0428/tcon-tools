...
var I18N = {
  ...
  'pat.applyLit':       { 'zh-TW': '（未選取 → 只套用到 {n} 個亮點，L0 不動）', 'en': '(none selected → applies to the {n} lit sub-pixels only, L0 untouched)', 'zh-CN': '（未选取 → 只套用到 {n} 个亮点，L0 不动）' },
  'pat.applyAllZero':   { 'zh-TW': '（全部為 L0 → 將套用至全部 {n} 個）', 'en': '(all L0 → applies to all {n})', 'zh-CN': '（全部为 L0 → 将套用至全部 {n} 个）' },
